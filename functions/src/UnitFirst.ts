...
export class UnitFirst {
    ...
    for (let i = 0; i < answerCount; ++i) {
      ...
      if (correctIDs.contains(i)) {
        ...
        if (Utils.withChance(0.5)) {
          // Create brand new function with 0.5 chance
          ...
          // Change second function of graph else
  ...
  static getSGtest(test_id: number, isSimple: boolean) {
    ...
    funcPoints.push(0);
    for (let i = 0; i < questionInterval - 1; i++) {
      temp += questionInterval;
      funcPoints.push(Math.round(temp));
    }
